!(function () {
  ...
  if (
    ...
    document.addEventListener("shopify:section:load", (e) => {
      if (![...e.target.classList].includes("t4s-main-store-locator")) return;
      const t = document.querySelector("[id^=t4s-store-locator-js]");
      if (t && t) {
  ...
  t.features.forEach((e, t) => {
    ...
    o.on("load", () => {
      o.addSource("places", { type: "geojson", data: t }),
        (function (t) {
          ...
          for (const e of t.features) {
            ...
            (s.id = `t4s-listing-${e.properties.id}`),
              (s.className = "t4s-store-locator__item");
            const i = s.appendChild(document.createElement("a"));
            (i.href = "#"),
              (i.className = "t4s-store-locator__title needsclick"),
              (i.id = `t4s-link-${e.properties.id}`),
              (i.innerHTML = `${r}`);
            ...
            (l.className = "t4s-rte"),
              ...
              s.addEventListener("click", function (e) {
                e.preventDefault();
                for (const e of t.features)
                  this.id === `t4s-listing-${e.properties.id}` && (n(e), a(e));
        ...
        (function () {
          for (const e of t.features) {
            ...
            (t.id = `t4s-marker-${e.properties.id}`),
              (t.className = "t4s-store-locator__marker"),
              s ||
                (t.innerHTML =
                  '<svg class="t4s-icon t4s-icon--store-locator" aria-hidden="true" focusable="false" role="presentation"><use href="#icon--store-locator-marker"></use></svg>'),
              new mapboxgl.Marker(t, { offset: [0, -23] })
  ...
  function n(e) {
    o.flyTo({ center: e.geometry.coordinates, zoom: 15 });
  }
  function a(e) {
    const t = document.getElementsByClassName("mapboxgl-popup");
    t[0] && t[0].remove();
    const { name: s, content: n } = e.properties,
      a = new mapboxgl.Popup({ closeOnClick: !1 })
        .setLngLat(e.geometry.coordinates)
        .setHTML(`<h3>${s}</h3><span>${n}</span>`)
        .addTo(o);
    a._container
      .querySelector(".mapboxgl-popup-content p")
      ?.classList?.add("t4s-rte"),
      window.width < 768 &&
        setTimeout(() => {
          a._container.scrollIntoView({ block: "end", behavior: "smooth" });
        }, 100);
  }
})();
